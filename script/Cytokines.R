...
library(report)
library(kableExtra)
# library(skedastic) # for breusch_pagan
library(bbmle)
...
# ggplot(data) + geom_point(aes(x = baseline, y = value, color = cytokine), alpha = 0.4)
# ggplot(data) + geom_point(aes(x = baseline, y = value/baseline, color = cytokine), alpha = 0.4)
# ggplot(data) + geom_point(aes(x = baseline, y = log(value/baseline), color = cytokine), alpha = 0.4)
...
bites_data <- read.csv(here("data","cumul_bites_control_groups_aggregated_corrected.csv"),
                       sep = ",", dec = ".")
colnames(bites_data)[3] <- "day"
my_data <- merge(data,bites_data, by = c("ID","day"))

# glmmTMB version ----
pdf(file = here("output","cytokines","repeated_bites","report_repeated_bites_controls_glmmTMB_no_LOD_AICc.pdf"))
...
pdf(file = here("output","cytokines","repeated_bites","report_repeated_bites_controls_glmmTMB_with_LOD_AICc.pdf"))
for(c in unique(data$cytokine)){
  df <- my_data[my_data$cytokine == c,]
  textplot(c, cex = 2)
  #########
...
# gam version ----
pdf(file = here("output","cytokines","repeated_bites","report_repeated_bites_controls_GAM_no_LOD_AICc.pdf"))
for(c in unique(data$cytokine)){
  df <- my_data[my_data$cytokine == c,]
  textplot(c, cex = 2)
  #########
...
pdf(file = here("output","cytokines","repeated_bites","report_repeated_bites_controls_GAM_with_LOD_AICc.pdf"))
for(c in unique(data$cytokine)){
  df <- my_data[my_data$cytokine == c,]
  textplot(c, cex = 2)
  #########
...
newDat$cumul_bites_7_previous_days <- 10 #mean(d_cyno$cumul_bites_4_previous_days)
newDat$ID <- "NV259" #sample(d_cyno$ID,1)
...
for(i in unique(d_cyno$ID)){
  ...
  new$cumul_bites_7_previous_days <- 10 #mean(df$cumul_bites_4_previous_days)
  new$ID <- i
...
d_cyno <- df[df$value != df$LOD & df$NHP == "Cyno",]
d_cyno$ID <- as.factor(d_cyno$ID)
# m_inf <- gam(log10(value) ~ bites_of_yesterday + s(cumul_bites_4_previous_days, k = 4) +
#                s(ID, bs = "re", k = 2),
#              data = d_cyno,
#              method = "ML")
m_inf <- gam(log10(value) ~ bites_of_yesterday +
...
newDat$bites_of_yesterday <- 0 #mean(d_cyno$bites_of_yesterday)
newDat$ID <- "NV259" #sample(d_cyno$ID,1)
pred_nl_den <- predict_gamm(m_inf, type = "response",
                            newdata = newDat, se = T,
                            re_form = NA)
...
for(i in unique(d_cyno$ID)){
  ...
  new$bites_of_yesterday <- 0 #mean(df$bites_of_yesterday)
  new$ID <- i
  newDat <- rbind(newDat,new)
}
pred_nl_den <- predict_gamm(m_inf, type = "response",
...
p_egf <- ggplot() + geom_line(data = pred_den, aes(x = bites, y = pred_cyto),
  ...
  geom_ribbon(data = pred_den, aes(x = bites, ymin = lwr, ymax = upr),
              fill = "lightgrey", alpha = 0.3) +
  # geom_point(data = d_cyno,
  #            aes(x = cumul_bites_4_previous_days, y = log10(value),
  #                col = ID),
  #            size = 2.5, alpha = 0.8) +
  labs(y = "Concentration (log<sub>10</sub> pg/\u03BCl)",
