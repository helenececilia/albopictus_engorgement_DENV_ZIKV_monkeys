...
zikv_cy$raw_file_inspected <- TRUE
...
# df_virus$int_duration <- minutes(chron(times. = df_virus$feed_duration,
#                                        format = "h:m:s")) # not working anymore
...
compare_means(prop ~ group_agg, data = test, method = "t.test")
...
# the results will be showed as brackets from bottom to top by default, or at specified coordinates
comp <- list(c("Squirrel.","Squirrel.Zika_virus"),
             c("Cyno.","Cyno.Zika_virus"),
             c("Squirrel.","Squirrel.Dengue_virus"),
             c("Cyno.","Cyno.Dengue_virus"),
             c("Cyno.","Squirrel."),
             c("Cyno.Dengue_virus","Squirrel.Dengue_virus"),
             c("Cyno.Zika_virus","Squirrel.Zika_virus"))

# DENV squirrel vs control squirrel : 3.2e-5 (****) needs to become ***
# DENV squirrel vs DENV cyno : 1.6 e-5 (****) needs to become ***
# threshold for **** needs to be 1.5e-5
# others need to be non signif : threshold 0.00071
...
p1 <- ggplot(test, aes(x = as.character(group_agg), y = prop)) +
  ...
  stat_compare_means(comparisons = comp,
                     ...
                     bracket.size = 1.02,
                     symnum.args = list(cutpoints = c(0, 0.000015, 0.0005, 0.0006, 0.0007, Inf),
                                        symbols = c("****", "***", "**", "*", "ns"))) +
  labs(y = "Proportion of<br>fed mosquitoes",
       x = "",
       fill = "Mosquito status",
  ...
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 30),
        axis.title.y = element_markdown(size = 38,
                                        margin = margin(r = 30, t = 390),
                                        vjust = 0),
        legend.position = c(1,0.27),
        plot.margin = unit(c(0,10,0,0.35), "cm"),
        legend.text = element_text(size = 38),
        legend.title = element_text(size = 38))
...
png(filename = here("output","mosq_feeding_behaviour","figures","Day0_no_duration.png"),
...
# TROPMED VERSION ----
comp <- list(c("Squirrel.","Squirrel.Dengue_virus"),
             c("Cyno.Dengue_virus","Squirrel.Dengue_virus"))
...
png(filename = here("output","mosq_feeding_behaviour","figures","TROPMED_Day0_no_duration.png"),
    width = 1600, height = 750)
print(p_draw1)
dev.off()
